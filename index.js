...
<head>
  ...
  <title>KudoFatura Bridge — Konfigurimi</title>
  <style>
...
app.post("/setup/install-service", (req, res) => {
  ...
  const createCmd = `sc create "KudoFatura Bridge" binPath= "${exePath}" start= auto DisplayName= "KudoFatura Fiscal Bridge"`;
  exec(createCmd, (err, _stdout, stderr) => {
    if (err) {
      return res.json({ success: false, error: stderr || err.message });
    }
    exec('sc start "KudoFatura Bridge"', () => {});
...
app.listen(PORT, HOST, () => {
  console.log(`[KudoFatura Bridge] Running on http://${HOST}:${PORT}`);
  console.log(`[KudoFatura Bridge] Config: ${CONFIG_PATH}`);

  const config = loadConfig();
  if (!config.configured) {
    console.log("[KudoFatura Bridge] First run detected — opening setup page...");
    setTimeout(() => openBrowser(`http://${HOST}:${PORT}/setup`), 800);
  } else {
    console.log(`[KudoFatura Bridge] Printer: ${config.brand} on ${config.port}`);
  }
});
